...
# Script de optimizacion completa y automatizada para Windows 11
# Autor: Basado en investigacion exhaustiva 2024-2025
# ============================================================================

#Requires -RunAsAdministrator
#Requires -Version 5.1

# Configuracion de colores para la interfaz
...
function Show-Banner {
    ...
    Write-ColorOutput "      Optimizacion Completa y Automatizada" "Cyan"
    Write-ColorOutput "========================================================" "Cyan"
    Write-ColorOutput ""
}

function Create-RestorePoint {
    Write-ColorOutput "`n>> Creando punto de restauracion del sistema..." "Yellow"
    
    try {
        Enable-ComputerRestore -Drive "C:\" -ErrorAction Stop
        Checkpoint-Computer -Description "Windows 11 Optimizer - Antes de optimizacion" -RestorePointType "MODIFY_SETTINGS" -ErrorAction Stop
        Write-ColorOutput "  [OK] Punto de restauracion creado exitosamente" "Green"
        return $true
    } catch {
        Write-ColorOutput "  [ERROR] Error al crear punto de restauracion: $_" "Red"
...
function Remove-Bloatware {
    ...
    Write-ColorOutput "  [OK] $removedApps aplicaciones eliminadas" "Green"
}
...
function Optimize-Registry {
    ...
    Write-ColorOutput "  [OK] $appliedTweaks optimizaciones aplicadas" "Green"
}
...
function Enable-UltimatePerformance {
    ...
    if ($output -match "GUID del plan de energia: ([a-f0-9-]+)") {
        $guid = $matches[1]
        ...
        Write-ColorOutput "  [OK] Plan Ultimate Performance activado (GUID: $guid)" "Green"
        "Plan Ultimate Performance activado: $guid" | Out-File -FilePath $LogFile -Append
    } else {
        Write-ColorOutput "  [ERROR] No se pudo habilitar Ultimate Performance" "Red"
    }
}
...
function Disable-Services {
    ...
    Write-ColorOutput "  [OK] $disabledServices servicios desactivados" "Green"
}

function Optimize-Network {
    param([string]$LogFile)
    
    Write-ColorOutput "`n>> Optimizando configuracion de red..." "Yellow"
    ...
    Write-ColorOutput "  [OK] Optimizaciones de red aplicadas" "Green"
}
...
function Optimize-Storage {
    ...
    # Configurar archivo de paginacion
    Write-ColorOutput "  - Configurando archivo de paginacion..." "Gray"
    ...
    Write-ColorOutput "    Configurado: Inicial=${initialSize}MB, Maximo=${maxSize}MB" "Green"
    ...
    Write-ColorOutput "  [OK] Optimizaciones de almacenamiento completadas" "Green"
}
...
function Disable-WindowsUpdate {
    ...
    # Configurar politicas de actualizacion
    ...
    # Configurar para notificar pero no descargar automaticamente
    ...
    Write-ColorOutput "  [OK] Windows Update configurado para control manual" "Green"
    "Windows Update configurado para control manual" | Out-File -FilePath $LogFile -Append
}

function Install-OptimizationTools {
    Write-ColorOutput "`n>> Instalando herramientas de optimizacion adicionales..." "Yellow"
    ...
    if (Test-InternetConnection) {
        ...
        try {
            ...
            Write-ColorOutput "  [OK] Win11Debloat instalado" "Green"
        } catch {
            Write-ColorOutput "  [ERROR] Error al instalar Win11Debloat" "Red"
        }
        ...
        try {
            ...
            Write-ColorOutput "  [OK] O&&O ShutUp10++ descargado en el escritorio" "Green"
        } catch {
            Write-ColorOutput "  [ERROR] Error al descargar O&&O ShutUp10++" "Red"
        }
    } else {
        Write-ColorOutput "  [ERROR] Sin conexion a Internet - omitiendo descargas" "Yellow"
    }
}
...
function Optimize-StartupPrograms {
    ...
    Write-ColorOutput "  [OK] $disabledCount programas de inicio desactivados" "Green"
}

function Create-UndoScript {
    param([string]$LogFile)
    
    Write-ColorOutput "`n>> Creando script de reversion..." "Yellow"
    
    $undoScript = @"
# Script de reversion para Windows 11 Optimizer
# Generado: $(Get-Date)
...
# Eliminar configuraciones de registro de optimizacion
...
Write-Host "Reversion completada. Reinicie el sistema." -ForegroundColor Green
"@
    
    $undoPath = "$env:USERPROFILE\Desktop\Win11_Optimizer_UNDO.ps1"
    $undoScript | Out-File -FilePath $undoPath -Encoding UTF8
    
    Write-ColorOutput "  [OK] Script de reversion guardado en: $undoPath" "Green"
}
...
function Show-Menu {
    Write-ColorOutput "`n========================================================" "Cyan"
    Write-ColorOutput "                    MENU DE OPTIMIZACION" "Cyan"
    Write-ColorOutput "========================================================" "Cyan"
    Write-ColorOutput "  1. Optimizacion COMPLETA (Recomendado)" "White"
    ...
    Write-ColorOutput "  7. Configurar Plan de Energia Ultimate" "White"
...
function Start-Optimization {
    ...
    if (-not $currentPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
        Write-ColorOutput "[ERROR] Este script requiere privilegios de Administrador" "Red"
    ...
    Write-ColorOutput "  [OK] Ejecutando como Administrador" "Green"
    Write-ColorOutput "  [OK] Windows 11 detectado" "Green"
    
    # Crear archivo de log
    $logFile = Create-LogFile
    Write-ColorOutput "  [OK] Archivo de log creado: $logFile" "Green"
        ...
        switch ($choice) {
            "1" {
                ...
                if (Create-RestorePoint) {
                    ...
                    Write-ColorOutput "      OPTIMIZACION COMPLETA FINALIZADA CON EXITO" "Green"
                    Write-ColorOutput "========================================================" "Green"
                    Write-ColorOutput "`n>> Recomendaciones:" "Yellow"
                    Write-ColorOutput "  - Reinicie el sistema para aplicar todos los cambios" "White"
                    Write-ColorOutput "  - El log completo esta en: $logFile" "White"
                    Write-ColorOutput "  - Script de reversion disponible en el escritorio" "White"
                }
            }
            "2" { 
                Remove-Bloatware -LogFile $logFile 
            ...
            default {
                Write-ColorOutput "Opcion no valida. Por favor, seleccione del 0 al 9" "Red"
            }
        }
